...
public class Model {
    ...
    private final ArrayList<Point> copy = new ArrayList<>();
    ...
    void scale(double dx, double dy, double x, double y) {
        clearAction();
        scaleAction = new ScaleAction(dx, dy, x, y);
        points.forEach(point -> {
            point.scale(dx, dy, x, y);
        });
    }
    ...
    void cancel() {
        if (moveAction != null) {
            ...
            points.forEach(point -> {
                point.revertScale(scaleAction.getDx(), scaleAction.getDy(), scaleAction.getXc(), scaleAction.getYc());
            });
